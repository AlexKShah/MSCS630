package com.alex.shah.encryptzip;

/**
 * file: AESCipher.java
 * author: Alex Shah
 * course: MSCS 630
 * assignment: Lab 5
 * due date: 4/18/18
 * version: 1.0
 * <p>
 * Given an initial key, generate 10 round keys according to AES definition
 * Use keys to encrypt entered plaintext with AES
 */

public class AESCipher {

    /**
     * Receive initial key, process key and generate 10 round keys
     *
     * @param keyHex - initial key as a String
     * @return keys - the 10 generated round keys
     */
    public static String aesRoundKeys(String keyHex) {
        // make key 4x4
        String[][] outHex = getMatrix(keyHex);

        // generate round keys
        String[][] keys = AESKeygen(outHex);
        String result = "";

        // add each line to result
        for (int count = 0; count < 11; count++) {
            for (int col = count * 4; col < count * 4 + 4; col++) {
                for (int row = 0; row < 4; row++) {
                    result += keys[row][col].toUpperCase();
                }
            }
            result += "\n";
        }
        return result;
    }

    /**
     * Given plaintext and key, encrypt plaintext using AES
     * @param pTextHex plaintext
     * @param keyHex key to encrypt with
     * @return ciphertext
     */
    public static String AES(String pTextHex, String keyHex) {
        String[][] plaintext = getMatrix(pTextHex);
        String[][] W = AESKeygen(getMatrix(keyHex));
        String[][] ciphertext = new String[4][4];
        String[][] currentKey = getMatrix(keyHex);

        //initial XOR
        ciphertext = AESStateXOR(plaintext, currentKey);

        //do AES steps, 10 rounds
        for (int round = 1; round < 11; round++) {
            //iterate round key
            for (int j = 0; j < 4; j++) {
                for (int k = 0; k < 4; k ++) {
                    currentKey[k][j] = W[k][(round*4)+j];
                }
            }

            //Nibble Substitution
            ciphertext = AESNibbleSub(ciphertext);

            //Shift Rows
            ciphertext = AESShiftRow(ciphertext);

            //Mix Columns except for last round
            if (round < 10) {
                ciphertext = AESMixColumn(ciphertext);
            }

            //XOR with current key
            ciphertext = AESStateXOR(ciphertext, currentKey);
        }
        ciphertext = rowtocol(ciphertext);
        return MatrixToString(ciphertext);
    }

    /**
     * XOR a 4x4 with another 4x4
     * @param sHex state matrix
     * @param keyHex key matrix
     * @return xor'd matrix result
     */
    public static String[][] AESStateXOR(String[][] sHex, String[][] keyHex) {
        String[][] out = new String[4][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                //perform xor
                out[i][j] = xorMe(sHex[i][j], keyHex[i][j]);
            }
        }
        return out;
    }

    /**
     * Substitute nibbles from SBox table
     * @param inStateHex input 2d matrix
     * @return 2d matrix with subsituted values
     */
    public static String[][] AESNibbleSub(String[][] inStateHex) {
        String[][] out = new String[4][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                //perform sbox substitution
                out[i][j] = aesSBox(inStateHex[i][j]);
            }
        }
        return out;
    }

    /**
     * Shift rows in increasing amounts
     * @param inStateHex input 2d matrix
     * @return output 2d shifted matrix
     */
    public static String[][] AESShiftRow(String[][] inStateHex) {
        String[][] out = new String[4][4];
        out[0] = inStateHex[0];
        for (int r = 1; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                // shift rows
                out[r][c] = inStateHex[r][(c + r) % 4];
            }
        }
        return out;
    }

    /**
     * Mix columns according to AES specs using galois fields
     * @param inStateHex input 2d matrix
     * @return mixed 2d matrix
     */
    public static String[][] AESMixColumn(String[][] inStateHex) {
        String[][] out = new String[4][4];
        String[][] out2 = new String[4][4];
        String[][] tmp = new String[4][4];
        int[] QQ = new int[4];
        int[] S = new int[4];

        // flip rows to cols
        tmp = rowtocol(inStateHex);

        // put the 2d values into
        // a temporary col based []
        for (int r = 0; r < 4; r++) {
            for (int c = 0; c < 4; c++) {
                QQ[c] = Integer.parseInt(tmp[r][c], 16);
            }

            // Galois field substitutions
            S[0] = mixcol2[QQ[0]] ^ mixcol3[QQ[1]] ^ QQ[2] ^ QQ[3];
            S[1] = QQ[0] ^ mixcol2[QQ[1]] ^ mixcol3[QQ[2]] ^ QQ[3];
            S[2] = QQ[0] ^ QQ[1] ^ mixcol2[QQ[2]] ^ mixcol3[QQ[3]];
            S[3] = mixcol3[QQ[0]] ^ QQ[1] ^ QQ[2] ^ mixcol2[QQ[3]];

            // build back into 2d matrix, don't lose 0's
            for (int k = 0; k < 4; k++) {
                String ans = String.format("%02x", S[k]);
                out[r][k] = ans;
            }
            // flip back
            out2 = rowtocol(out);
        }
        return out2;
    }

    /**
     * Convert a 2d matrix to a String for print out
     * @param matrix a 2d matrix to convert
     * @return a String made from 2d matrix
     */
    public static String MatrixToString(String[][] matrix) {
        String ctxt = "";
        for (int row = 0; row < 4; row++) {
            ctxt += matrix[row][0];
            ctxt += matrix[row][1];
            ctxt += matrix[row][2];
            ctxt += matrix[row][3];
        }
        return ctxt.toUpperCase();
    }

    /**
     * flip flop rows to columns to make a 2d matrix transposed
     * @param matrix input matrix to flipflop
     * @return resulting 2d matrix, transposed
     */
    public static String[][] rowtocol(String[][] matrix) {
        String[][] tmp = new String[4][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                //transpose
                tmp[i][j] = matrix[j][i];
            }
        }
        return tmp;
    }

    /**
     * converts a String key to 4x4 String[][] hex representation
     *
     * @param key - key as String to be converted
     * @return cipher - String[][] hex format of key
     */
    static String[][] getMatrix(String key) {
        String[][] cipher = new String[4][4];
        int row = 0;
        int col = 0;

        // Iterate by 2s, add 2 chars to represent hex
        for (int i = 0; i < key.length(); i += 2) {
            cipher[row][col] = "" + key.charAt(i) + key.charAt(i + 1);
            row++;

            if (row == 4) {
                row = 0;
                col++;
            }

        }
        return cipher;
    }

    /**
     * Performs steps to get AES round keys
     *
     * @param inHex - original key as 4x4 matrix
     * @return keys - original key + 10 round keys
     */
    static String[][] AESKeygen(String[][] inHex) {
        String[][] W = new String[4][44];

        // round 0
        // first 4 columns of W are first 4 columns of inHex
        for (int i = 0; i < 4; i++) {
            W[i][0] = inHex[i][0];
            W[i][1] = inHex[i][1];
            W[i][2] = inHex[i][2];
            W[i][3] = inHex[i][3];
        }

        // round 4 onwards
        for (int j = 4; j < 44; j++) {

            // Column not multiple of 4
            if (j % 4 != 0) {

                // 3a: w(j) = w(j − 4) XOR w(j − 1)
                for (int row = 0; row < 4; row++) {
                    W[row][j] = xorMe(W[row][j - 4], W[row][j - 1]);
                }

            } else {
                // 3b
                // wnew = [ (Rcon(i) XOR Sbox(w1,j−1)), Sbox(w2,j−1), Sbox(w3,j−1), Sbox(w0,j−1) ]
                // w(j) = w(j − 4) XOR wnew

                // wnew = [ (Rcon(i) XOR Sbox(w1,j−1)), Sbox(w2,j−1), Sbox(w3,j−1), Sbox(w0,j−1) ]
                String[] wnew = new String[4];
                String rconValue = aesRcon(j);
                wnew[0] = xorMe(rconValue, aesSBox(W[1][j - 1]));
                wnew[1] = aesSBox(W[2][j - 1]);
                wnew[2] = aesSBox(W[3][j - 1]);
                wnew[3] = aesSBox(W[0][j - 1]);

                // w(j) = w(j − 4) XOR wnew
                for (int row = 0; row < 4; row++) {
                    W[row][j] = xorMe(W[row][j - 4], wnew[row]);
                }

            }
        }
        return W;
    }

    /**
     * Substitutes initial hex value with SBox defined subsitution
     *
     * @param inHex - Character to subsitute
     * @return xAsHex - SBox substituted character in hex
     */
    static String aesSBox(String inHex) {
        // hex to int for index
        int index = Integer.parseInt(inHex, 16);
        char x = sbox[index];
        // convert to capitalized String
        String xAsHex = Integer.toHexString((int) x).toUpperCase();
        return xAsHex;
    }

    /**
     * Substitutes value based on which round we're on
     *
     * @param round - the current round
     * @return xAsHex- Substituted value
     */
    static String aesRcon(int round) {
        char x = rcon[(int) Math.floor(round / 4)];
        String xAsHex = Integer.toHexString((int) x).toUpperCase();
        return xAsHex;
    }

    /**
     * Performs an xor on two strings
     *
     * @param A - left side of xor
     * @param B - right side of xor
     * @return ans - the xor result as a string
     */
    static String xorMe(String A, String B) {
        int A_int = Integer.parseInt(A, 16);
        int B_int = Integer.parseInt(B, 16);
        int ans_int = A_int ^ B_int;
        String ans = String.format("%02x", ans_int);
        return ans;
    }

    // SBox Substitutions
    private static final char[] sbox = {
            
    };

    // Rcon Substitutions
    private static final char[] rcon = {
            
    };

    // Mix columns Galois Field substitutions, 2-table
    private static final int[] mixcol2 = {
            
    };

    // Mix columns Galois Field substitutions, 3-table
    private static final int[] mixcol3 = {
            
    };
}